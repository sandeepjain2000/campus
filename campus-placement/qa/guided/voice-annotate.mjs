...
const CONFIG_PATH = path.join(__dirname, 'guided-voice-config.json');
...
export function announceStep({ stageKey, role, text, auto }) {
  ...
  if (result.status !== 0) {
    ...
    console.warn(`  [voice] ${msg || 'annotation failed — install: pip install -r qa/guided/requirements-voice.txt'}`);
