...
const manifestPath = path.join(__dirname, 'use-case-runners.json');
...
function main() {
  ...
  if (!slug || slug === '--help' || slug === '-h') {
    ...
    console.log('       run_use_case_auto_voice.bat <slug>\n');
    console.log('Slugs:\n');
    for (const c of manifest.cases) {
      console.log(`  ${c.slug.padEnd(32)} ${c.title}`);
    }
    console.log('\nOne-time voice setup: pip install -r qa/guided/requirements-voice.txt');
