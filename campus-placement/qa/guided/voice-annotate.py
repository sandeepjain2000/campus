...
_CONFIG_PATH = _SCRIPT_DIR / "guided-voice-config.json"
...
def _transcript_path(voice: dict, stage_key: str) -> Path:
    base = _resolve_dir(voice, "transcript_dir", "qa/guided/voice/transcripts")
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return base / f"{stamp}_{stage_key}.txt"


def _audio_path(voice: dict, stage_key: str) -> Path:
    base = _resolve_dir(voice, "audio_dir", "qa/guided/voice/audio")
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return base / f"{stamp}_{stage_key}.mp3"


def _append_manifest(voice: dict, entry: dict) -> None:
    rel = voice.get("manifest_file", "qa/guided/voice/voice_manifest.jsonl")
...
async def _synthesize_edge_tts(text: str, output_mp3: Path, voice_id: str, rate: str, volume: str) -> bool:
    try:
        ...
        print("edge-tts not installed — transcript only. pip install -r qa/guided/requirements-voice.txt", file=sys.stderr)
        return False
