...
**Guided Runner (partial auto testing):** open **[Developer](/developer)** from the landing page (Developer → Developer notes), or see [`qa/guided/RUNNER_QUICKSTART.md`](guided/RUNNER_QUICKSTART.md) / `npm run test:guided:help`.
...
Structured manual QA for **CSV upload/download** and **cross-view data consistency**. Use with `manual_session_log.csv` and `csv_screens_inventory.csv` in this folder.
...
5. Open `manual_session_log.csv` and fill the session header row.
...
For each export screen (see `csv_screens_inventory.csv`):
...
- [ ] `manual_session_log.csv` completed for the session  

See also `uat_signoff_checklist_audit_and_assessments.csv` for audit/assessment-specific rows.
...
```bash
node qa/guided/run-guided.mjs --playbook internships-employer-publish
```
...
- `qa/guided/focus-areas.json` — step-by-step runner data
- `qa/focus_areas_catalog.csv` — spreadsheet for testers (case ID, one-liner, command)
...
```bash
node qa/guided/run-guided.mjs --focus EI-03
...
```bash
node qa/guided/run-guided.mjs --section internships.employer
node qa/guided/run-guided.mjs --section internships
node qa/guided/run-guided.mjs --section placement-drives.college
npm run test:guided:internships
...
Log observations in `manual_session_log.csv`.
...
```bash
node qa/guided/run-guided.mjs --uc UC-002
node qa/guided/run-guided.mjs --uc UC-007 --base-url https://campus-placement-omega.vercel.app
```
...
Use case definitions: `qa/guided/use-cases.json` (edit to add flows).
...
When logging a failure in `manual_session_log.csv`:
...
| File | Purpose |
|------|---------|
| `manual_session_log.csv` | Per-step log for each session |
| `csv_screens_inventory.csv` | Upload/export screens by role |
| `C-2-*.json` … `C-5-*.json` | Detailed test case definitions |
| `uat_signoff_checklist_audit_and_assessments.csv` | Audit & assessment sign-off |
| `routes-by-role.js` | Dashboard routes for blank-screen automation |
