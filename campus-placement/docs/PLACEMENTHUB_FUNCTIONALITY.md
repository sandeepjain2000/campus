...
**Word export:** [`PLACEMENTHUB_FUNCTIONALITY.docx`](PLACEMENTHUB_FUNCTIONALITY.docx) — diagrams embedded as PNG (regenerate: `python scripts/md_to_docx.py`; refresh diagrams: `python scripts/md_to_docx.py --force-diagrams`)
...
Beyond core placement, the product supports **internships, short projects, hackathons, alumni hiring, sponsorships, startup seed funding, guest faculty, and campus calendars** — so industry engagement and final placements live in one place rather than scattered spreadsheets and email threads.
...
Students apply from **Browse** screens; status is tracked under **My Applications** (separate tabs per type). Employers manage listings under **Student Opportunities**; colleges monitor **Applications** and program boards.
...
These share the same tenant and partnership model as hiring but do not replace the application/offer pipeline.
