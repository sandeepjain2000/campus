...
const MOCK_DATA = {
  ...
  recentApplications: [
    { id: 1, name: 'Arjun Verma', role: 'SDE', college: 'IIT Mumbai', cgpa: 8.72, status: 'shortlisted', appliedAt: '2026-09-08' },
    { id: 2, name: 'Sneha Iyer', role: 'SDE', college: 'IIT Mumbai', cgpa: 9.15, status: 'in_progress', appliedAt: '2026-09-08' },
    { id: 3, name: 'Rohan Patel', role: 'Full Stack Dev', college: 'IIT Mumbai', cgpa: 7.65, status: 'applied', appliedAt: '2026-09-11' },
    { id: 4, name: 'Kavya Reddy', role: 'SDE', college: 'IIT Mumbai', cgpa: 8.45, status: 'selected', appliedAt: '2026-09-08' },
  ],
  upcomingDrives: [
    { id: 1, college: 'IIT Mumbai', role: 'SDE Drive', date: '2026-09-15', type: 'on_campus', status: 'scheduled' },
    { id: 2, college: 'NIT Trichy', role: 'Full Stack Drive', date: '2026-09-22', type: 'virtual', status: 'approved' },
  ]
};
...
export async function GET(request) {
  try {
    ...
    // MOCK DATA since actual live data is low
    // We append the campusId string into the mock to make it "dynamic" for now
    const MOCK_DATA = {
      stats: {
        activeJobs: 2,
      ...
      recentApplications: [
        { id: 1, name: 'Arjun Verma', role: 'SDE', college: 'Campus Selection', cgpa: 8.72, status: 'shortlisted', appliedAt: new Date().toISOString() },
        { id: 2, name: 'Sneha Iyer', role: 'SDE', college: 'Campus Selection', cgpa: 9.15, status: 'in_progress', appliedAt: new Date().toISOString() },
      ],
      upcomingDrives: [
        { id: 1, college: 'Selected Campus', role: 'SDE Drive', date: '2026-09-15', type: 'on_campus', status: 'scheduled' }
      ]
    };

    // Attempt real database queries correctly scoped to campusId
    try {
      ...
      return NextResponse.json({
        ...
        recentApplications: appsQuery.rows.length ? appsQuery.rows : MOCK_DATA.recentApplications,
        upcomingDrives: drivesQuery.rows.length ? drivesQuery.rows : MOCK_DATA.upcomingDrives,
      });
    } catch(e) {
      console.log('Real query failed, fallback to mock', e);
