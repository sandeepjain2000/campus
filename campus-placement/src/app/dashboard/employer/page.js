...
import { LayoutDashboard, Briefcase, FileText, CheckCircle, Send, Users, Calendar, ArrowRight } from 'lucide-react';
    ...
    <div className="animate-fadeIn">
      <div className="page-header">
        <div className="page-header-left">
          ...
          <p className="text-secondary text-sm">Managing recruitment for <strong style={{ color: 'var(--text-primary)' }}>{activeCampus.name}</strong></p>
        </div>
        <Link href="/dashboard/employer/jobs" className="btn btn-primary">+ Post New Job</Link>
      </div>

      {/* Stats */}
      <div className="grid grid-4" style={{ marginBottom: '1.5rem' }}>
        <div className="stats-card">
          <div className="stats-card-icon indigo"><Briefcase size={24} strokeWidth={1.5} /></div>
          <div className="stats-card-value">{stats.activeJobs}</div>
          <div className="stats-card-label">Active Jobs</div>
        </div>
        <div className="stats-card green">
          <div className="stats-card-icon green"><FileText size={24} strokeWidth={1.5} /></div>
        ...
        <div className="stats-card amber">
          <div className="stats-card-icon amber"><CheckCircle size={24} strokeWidth={1.5} /></div>
        ...
        <div className="stats-card rose">
          <div className="stats-card-icon rose"><Send size={24} strokeWidth={1.5} /></div>
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            {['Applied', 'Shortlisted', 'Interview', 'Selected'].map((stage, i) => {
              ...
              const bgColors = ['rgba(2, 132, 199, 0.05)', 'rgba(79, 70, 229, 0.05)', 'rgba(217, 119, 6, 0.05)', 'rgba(5, 150, 105, 0.05)'];
              return (
                <div key={stage} style={{ flex: 1, textAlign: 'center', padding: '1rem', borderRadius: 'var(--radius-md)', background: bgColors[i] }}>
                  <div style={{ fontSize: '1.25rem', fontWeight: 700, color: colors[i] }}>{counts[i]}</div>
                  <div className="text-xs font-semibold" style={{ marginTop: '0.25rem', color: 'var(--text-secondary)' }}>{stage}</div>
                </div>
              );
            })}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {upcomingDrives.map((drive, index) => (
              <div key={drive.id} style={{ 
                ...
                <div>
                  <div className="font-semibold text-sm" style={{ color: 'var(--text-primary)' }}>{drive.college} - {drive.role}</div>
            ...
            {upcomingDrives.length === 0 && (
              <div className="text-sm text-secondary" style={{ padding: '2rem 1rem', textAlign: 'center', background: 'var(--bg-secondary)', borderRadius: 'var(--radius-md)' }}>
                ...
                <tr key={app.id}>
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      <div className="avatar avatar-sm" style={{ background: 'linear-gradient(135deg, var(--gray-100), var(--gray-200))', color: 'var(--gray-700)', fontWeight: 600 }}>
                        {app.name.split(' ').map(n=>n[0]).join('')}
                      </div>
                      <span className="font-semibold text-sm" style={{ color: 'var(--text-primary)' }}>{app.name}</span>
                    </div>
                  </td>
                  <td><span className="text-sm font-medium">{app.role}</span></td>
                  <td><span className="text-sm text-secondary">{app.college}</span></td>
                  <td><span className="font-semibold text-sm" style={{ color: 'var(--text-primary)' }}>{app.cgpa}</span></td>
                  <td><span className={`badge badge-${getStatusColor(app.status)} badge-dot`}>{formatStatus(app.status)}</span></td>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: '0.25rem' }}>
                      ...
                      <button className="btn btn-primary btn-sm" onClick={() => alert("Feature coming soon! (Wireframe Action)")}>Shortlist</button>
                    </div>
                  </td>
                </tr>
              ))}
