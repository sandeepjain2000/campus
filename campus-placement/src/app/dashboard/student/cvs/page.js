...
export default function StudentCvsPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR('/api/student/cvs?includeArchived=1', fetcher);
  ...
  const patchCv = async (id, body) => {
    const res = await fetch(`/api/student/cvs/${id}`, {
      method: 'PATCH',
                    ...
                    <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                      {editingId === cv.id ? (
                        ...
                        <>
                          ...
                          <a
                            className="btn btn-ghost btn-sm"
                            href={`/api/student/cvs/${encodeURIComponent(cv.id)}/view`}
                            target="_blank"
