...
export default function CollegeEmployersPage() {
  ...
  return (
    <div className="animate-fadeIn" style={{ paddingBottom: '3rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    ...
                    {emp.status === 'approved' && (
                      <>
                        ...
                        <button type="button" className="btn btn-ghost btn-sm" style={{ color: '#dc2626', border: '1px solid #fecaca', display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.8rem' }} onClick={() => setRevokeTarget({ id: emp.employer_id, name: emp.name })} disabled={processingId === emp.employer_id}>
      ...
      <style>{`
        .employers-mobile-cards { display: none; }
        @media (max-width: 768px) {
          .employers-table-wrap { display: none; }
          .employers-mobile-cards { display: block; }
        }
      `}</style>
    </div>
  );
