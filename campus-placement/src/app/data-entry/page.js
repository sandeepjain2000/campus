    ...
    items: [
      {
        ...
        hint: 'TechCorp, GlobalSoft, Infosys, Innovent Labs, FinEdge only',
...
const FLAT_ACTIONS = ACTION_GROUPS.flatMap((g) => g.items);
...
const PURGE_LIST_KEY_BY_ENTITY = Object.fromEntries(
  PURGE_GROUPS.map((g) => [g.entityType, g.key]),
);

function purgeRemainingCount(candidates) {
  if (!candidates) return 0;
  return PURGE_GROUPS.reduce((n, g) => n + (candidates[g.key] || []).length, 0);
}

function purgeOptionKey(item) {
  return `${item.entityType}:${item.entityId}`;
}

/** Flat list for purge dropdown, optionally filtered by entity type. */
function flattenPurgeCandidates(candidates, typeFilter = 'all') {
...
function formatRunForDownload(run) {
...
export default function DataTesterHubPage() {
  ...
  return (
    <div className="demo-tester-page">
      <div className="demo-tester-wrap">
        <header className="demo-tester-header">
          <h1>
            <Database size={20} aria-hidden /> Demo Data Tester
          </h1>
          <p>
        ...
        <section className="demo-tester-purge">
